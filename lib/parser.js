...
function applyModifiers(modifier, config) {
  ...
  switch (modifier) {
    ...
    case `${config.modifiers.string}`:
      value = '""';
      break;
    case `${config.modifiers.check}`:
      value = 'Check values.yaml';
      break;
    default:
      value = undefined;
  }
 ...
 * }
 */
function parseMetadataComments(valuesFilePath, config, envParameters) {
  const data = fs.readFileSync(valuesFilePath, 'UTF-8');
  const lines = data.split(/\r?\n/);

  const parsedValues = [];
  const envParamRegex = new RegExp(`^\\s*${config.comments.format}\\s*${config.tags.param}\\s*([^\\s]+)\\s*([^\\s]+)\\s*(\\[.*\\])?\\s*(.*)$`);
  ...
  lines.forEach((line) => {
    // Parse param and env line
    let paramMatch;
    if (envParameters) paramMatch = line.match(envParamRegex);
    else paramMatch = line.match(paramRegex);
    if (paramMatch && paramMatch.length > 0) {
      if (envParameters) {
        parsedValues.push({
          name: paramMatch[1],
          env: paramMatch[2],
          value: paramMatch[3] ? applyModifiers(paramMatch[3].split('[')[1].split(']')[0], config) : undefined,
          modifier: !!paramMatch[3],
